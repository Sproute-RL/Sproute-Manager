...
DWORD Memory::GetIDByName(const char *name)
{
    ...
    if (Process32First(snapshot, &pe))
    {
        do
        {
            if (wcscmp(pe.szExeFile, ConvertToWide(name).c_str()) == 0)
            {
...
uintptr_t Memory::AllocateMemory(size_t size, uintptr_t baseAddress)
{
    if (!pHandle)
    {
    ...
    LPVOID allocatedMemory = VirtualAllocEx(pHandle, (PVOID)baseAddress, size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
...
bool Memory::ChangePageProtection(uintptr_t address, size_t size, DWORD &oldProtection, DWORD newProtection) 
{
    if (pHandle == INVALID_HANDLE_VALUE)
    {
    ...
    if (!VirtualProtectEx(pHandle, (PVOID)address, size, newProtection, &oldProtection))
    {
